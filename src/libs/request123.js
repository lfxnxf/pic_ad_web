...
const buildParams = (params = {}) => {
  // 公共参数
  const commonParams = {
    // appkey: config.VUE_APP_CONFIG_APP_ID,
    // channel_id: config.VUE_APP_CONFIG_APP_CHANNEL_ID,
  ...
  Object.keys(buildParams).forEach((key) => {
    if (buildParams[key] === null || buildParams[key] === undefined) {
      delete buildParams[key]
    }
  })
  // 签名
  // buildParams.sign = sign(buildParams, config.VUE_APP_SIGN_API, config.VUE_APP_CONFIG_APP_KEY)
...
instance.interceptors.request.use(
  (config) => {
    ...
    token && (config.data.sessionid = token)
...
instance.interceptors.response.use(
  ...
  (res) => {
    if (res.status === 200) {
      if (hasKey(res.data, 'return_code') && res.data.return_code === 0) {
        // let data = res.data.data;
        return Promise.resolve(res)
      } else {
        //   tip(res.data.return_message);
