...
var Methods = {
          ...
          if (isFull) {
            ...
            if (treeConfig) {
              if (!_xeUtils.default.findTree(afterFullData, function (row) {
                return getRowid(_this30, row) === oldRowid;
              }, treeOpts)) {
                reserveSelection.push(oldRow);
              }
            } else {
  ...
  computeScrollLoad: function computeScrollLoad() {
    ...
    return this.$nextTick().then(function () {
      var sYOpts = _this44.sYOpts,
          sXOpts = _this44.sXOpts,
          scrollXLoad = _this44.scrollXLoad,
          scrollYLoad = _this44.scrollYLoad,
          scrollXStore = _this44.scrollXStore,
          scrollYStore = _this44.scrollYStore; // 计算 X 逻辑

      if (scrollXLoad) {
        var _computeVirtualX3 = computeVirtualX(_this44),
      ...
      var _computeVirtualY = computeVirtualY(_this44),
  ...
  scrollToTreeRow: function scrollToTreeRow(row) {
    ...
    if (treeConfig) {
      ...
      if (matchObj) {
        ...
        nodes.forEach(function (row, index) {
          if (index < nodes.length - 1 && !_this46.isTreeExpandByRow(row)) {
            rests.push(_this46.setTreeExpand(row, true));
          }
        });
      }
    }
    ...
    return Promise.all(rests).then(function () {
      return _tools.DomTools.rowToVisible(_this46, row);
    });
  },
  ...
  updateStatus: function updateStatus(scope, cellValue) {
    ...
    return this.$nextTick().then(function () {
      var $refs = _this47.$refs,
          editRules = _this47.editRules,
          validStore = _this47.validStore;
      ...
      if (scope && $refs.tableBody && editRules) {
        ...
        if (_this47.hasCellRules(type, row, column)) {
          var cell = _this47.getCell(row, column);

          if (cell) {
            return _this47.validCellRules(type, row, column, cellValue).then(function () {
              if (customVal && validStore.visible) {
                setCellValue(row, column, cellValue);
              }

              _this47.clearValidate();
  ...
  setMergeCells: function setMergeCells(merges) {
    ...
    return this.$nextTick().then(function () {
      return _this48.updateCellAreas();
    });
  },
